...
releasesRoutes.get("/releases", async (r, sn) => { const org = requireOrgId(r, sn); if (!org) return; sn.json((await releasesService.list(org)).data); });
releasesRoutes.post("/releases", async (r, sn) => { const org = requireOrgId(r, sn); if (!org) return; sn.status(201).json((await releasesService.create(org, s.parse(r.body))).data); });
...
releasesRoutes.patch("/releases/:id", async (r, sn) => { const org = requireOrgId(r, sn); if (!org) return; const releaseId = r.params.id; if (!releaseId) return sn.status(400).json({ error: "releaseId is required" }); sn.json((await releasesService.update(org, releaseId, s.partial().parse(r.body))).data); });
releasesRoutes.delete("/releases/:id", async (r, sn) => { const org = requireOrgId(r, sn); if (!org) return; const releaseId = r.params.id; if (!releaseId) return sn.status(400).json({ error: "releaseId is required" }); await releasesService.delete(org, releaseId); sn.status(204).send(); });
